    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < logic.getRows(); i++) {
            for (int j = 0; j < logic.getColumns(); j++) {
                ...
                int finalJ = j;
                btn.setMouseClick(() -> {
                    logic.reveal(finalI, finalJ); //another swap
                    redraw(logic);
                });
                btn.setContextClick(() -> {
                    logic.toggleFieldState(finalI, finalJ); //another swap
                    redraw(logic);
                });
            }
        }
    ...
    private static void redraw(Minesweeper logic) {
        ...
        for (int i = 0; i < cells.length; i++) {
            for (int j = 0; j < cells[0].length; j++) {
                int field = logic.getField(i, j); //fixed bug - swapped i and j indices?
                ...
                if (field == 0) {
                    ...
                    int bombCount = logic.getAdjacentBombCount(i, j); //another swap
                    if (bombCount > 0) {
